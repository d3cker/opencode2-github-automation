...
import { Blocked, type Task } from "../src/dispatcher.js";
...
test("transport failure while looking up a session never creates a duplicate", async () => {
  ...
  const t = task(); await assert.rejects(new OpenCodeExecutor(ctx, options, new AbortController().signal).run(t, async p => { Object.assign(t, p); }), /network/);
...
test("abort of a running wait interrupts the server session", async () => {
  ...
  await assert.rejects(new OpenCodeExecutor(ctx, options, controller.signal).run({ ...task(), sessionID: "ses_test", promptAttempted: true }, async () => {}));
...
test("real git worktree isolates a fix, verifies, commits and pushes to a local bare remote", async () => {
  ...
  try {
    ...
    const repo = { ...options.repositories[0]!, directory: checkout, checks: [[process.execPath, "-e", 'if (require("fs").readFileSync("counter.txt", "utf8") !== "fixed\\n") process.exit(1)']] };
