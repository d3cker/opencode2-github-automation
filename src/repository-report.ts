...
export function repositoryDetails(row: RepositoryRow) {
  ...
  const tasks = d?.tasks.filter(t => t.repo === row.repo && t.status !== "closed" && (t.status === "closing" || t.prState !== "closed" && t.phase !== "merged"));
  const counts = tasks ? `Active: ${tasks.filter(t => t.key === d?.activeTask).length} · Scheduled: ${tasks.filter(t => ["ready", "retry_wait"].includes(t.status) && t.key !== d?.activeTask).length} · Waiting: ${tasks.filter(t => t.status === "waiting").length} · Blocked/failed: ${tasks.filter(t => ["blocked", "failed"].includes(t.status)).length} · Closing: ${tasks.filter(t => t.status === "closing").length}` : "Task counts: unavailable";
  return [
    ...
    ...(tasks?.filter(t => ["blocked", "failed", "closing"].includes(t.status)).map(t => `${t.key}: ${t.status} · ${t.error ?? "Stopping sessions"}`) ?? []),
  ].map(plain).join("\n");
}
