...
export type SetupValues = { model?: string; trigger?: string; signature?: string; authors?: string[]; everySeconds?: number; check?: string[] | false; autoMerge?: { enabled: boolean; method: "merge" | "squash" | "rebase" } };
export async function configure(question: Question, defaults: { login: string; model?: string; check?: string[] }, supplied: SetupValues = {}) {
  ...
  return EasyOptions.parse({ model, trigger, signature, authors, everySeconds, autoMerge: { enabled, method }, check });
}
