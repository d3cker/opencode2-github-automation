...
import { Blocked, type Executor, type Task } from "./dispatcher.js";
...
export class GitWorkspace {
  ...
  async prepare(task: Task, repo: Repository) {
    ...
    await this.git(repo.directory, "fetch", "origin", `refs/heads/${repo.baseBranch}:refs/remotes/origin/${repo.baseBranch}`);
...
export class OpenCodeExecutor implements Executor {
  ...
  constructor(private ctx: Plugin.Context, private options: GithubOptions, private signal: AbortSignal) {
  ...
  prepare(task: Task, repo: Repository) { return this.git.prepare(task, repo); }
