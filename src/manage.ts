...
const commands = ["status", "scan", "run", "pause", "resume", "retry", "restartworkflow", "cancelround", "resumetracking"];
if (!command || !commands.includes(command) || !directory || ["run", "pause", "resume", "retry", "restartworkflow", "cancelround", "resumetracking"].includes(command) && !argument) {
  console.error("Usage: node dist/manage.js <status|scan|run|pause|resume|retry|restartworkflow|cancelround|resumetracking> <owner-directory> [job-id|issue-key] [--restart-session]");
  process.exitCode = 1;
} else {
  try {
    ...
    switch (command) {
      ...
      case "cancelround": result = await github.cancelround({ key: argument! }, request); break;
      case "resumetracking": result = await github.resumetracking({ key: argument! }, request); break;
